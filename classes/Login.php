...
class Login extends DBConnection {
    ...
    public function login(){
        ...
        $stmt = $this->conn->prepare("SELECT * from users where BINARY username = ? and BINARY password = ? and `type` != 3 ");
        ...
        if ($result->num_rows > 0) {
            ...
            return json_encode(array('status' => 'incorrect', 'last_qry' => "SELECT * from users where BINARY username = '$username' and BINARY password = md5('$password') "));
        }
    }
    ...
    function login_agent(){
        ...
        $stmt = $this->conn->prepare("SELECT * from agent_list where BINARY email = ? and BINARY `password` = ? and delete_flag = 0 ");
        $password = md5($password);  // Assuming password is still hashed with MD5
