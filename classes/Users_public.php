...
class UsersPublic extends DBConnection {
    ...
    public function reset_password_user($return_array = false) {
        $username = $_POST['username'] ?? '';
        $security_question = $_POST['security_question'] ?? '';
        $security_answer = $_POST['security_answer'] ?? '';
        $new_password = $_POST['new_password'] ?? '';
        ...
        // Check if user exists and is type 2 (user)
        ...
        $update = $this->conn->prepare("UPDATE users SET password = ? WHERE username = ? AND type = 2");
        $update->bind_param("ss", $hashed_password, $username);
